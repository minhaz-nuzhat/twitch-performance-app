...
function ProgressChartTooltip({ active, label, payload }) {
  ...
  const formatValue = (entry) => {
    if (entry.dataKey === 'score') return `${entry.value} points`
    if (entry.dataKey === 'load') return `${entry.value} AU`
    if (entry.dataKey === 'rpe') return `${entry.value} / 10`
    return entry.value
  }

  return (
    <div className="pointer-events-none max-w-52 rounded-lg border border-tp-border-bright bg-tp-card px-3 py-2 shadow-[0_8px_28px_rgba(0,0,0,0.85)]">
      <p className="text-tp-white text-xs font-semibold mb-1.5">{label}</p>
      {payload.map((entry) => (
        <div key={entry.dataKey} className="flex items-center justify-between gap-4 text-xs">
          <span className="flex items-center gap-1.5 text-tp-soft"><span className="w-2 h-2 rounded-full" style={{ backgroundColor: entry.color }} />{entry.name}</span>
          <span className="text-tp-white font-mono font-bold whitespace-nowrap">{formatValue(entry)}</span>
        </div>
      ))}
    </div>
  )
